...
options(timeout = 1200) #timeout in download.file

force_download_env <- toupper(Sys.getenv("FORCE_DOWNLOAD"))
force_download <- force_download_env == "TRUE"
...
if (length(list.files(geo.dir)) != 9 || force_download) {
  ...
  GEOquery::getGEOSuppFiles("GSE190992", 
                          ...
                          fetch_files = TRUE, 
                          makeDirectory = FALSE) 
#                          filter_regex = "!(*GSE190992_AIFI-scRNA-PBMC-FinalData.RDS.gz$)")
...
if (!dir.exists(file.path(data.dir, "hdf5data"))) {
  data <- readRDS(
  gzfile(file.path(geo.dir, "GSE190992_AIFI-scRNA-PBMC-FinalData.RDS.gz")))
