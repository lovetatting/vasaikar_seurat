...
#debug
DEBUG_env <- toupper(Sys.getenv("DEBUG"))
...
time_it <- function(func, ...) {
  ...
  args <- sapply(match.call()[-1], deparse)  # Capture and deparse arguments
  args_str <- paste(names(args), args, sep = "=", collapse = ", ")  # Format arguments
  ...
  end_time <- Sys.time()
  duration <- round(end_time - start_time)  # Duration rounded to whole seconds
  
  log_info(glue::glue("Function {func_name}({args_str}) executed in {duration} seconds."))
  log_info('Current RAM usage: {mem_used()}')
...
geo.dir <- create_directory(file.path(data, "GEO"), 'for GEO files')
hdf5.dir <- create_directory(file.path(data.dir, "hdf5"), 'for HDF5 files')
...
data.v5.raw.path <- file.path(hdf5.v5.raw.dir, "data-v5-raw")


```

```{r precondition_raw_data}

geo.accession = "GSE190992"

log_info('The GEO is: {GEO}')
...
get_geo_supp_files <- function(file_names, geo.dir, geo.accession) {
  for (file_name in file_names) {
    ...
    if (file.exists(file_path)) {
      ...
      start_time <- Sys.time()
      GEOquery::getGEOSuppFiles(GEO = geo_accession, 
                                destdir = geo.dir, 
...
get_geo_supp_files(geo_suppl_files, geo.dir, GEO)
...
```{r seurat_v4_raw_unzip}

if(!file.exists(rds.file)) {
  log_info("Unzipping RDS file for seurat v4 object at {rds.file}")
  start_time <- Sys.time()
  
  gunzip(rds.file.gz)
  
  end_time <- Sys.time()
  unzip_duration <- end_time - start_time
  log_info("Unzipped {rds.file} in {unzip_duration} seconds.")
}
...
if(! file.exists(data.v5.raw.path)) {
  log_info('Cannot find file at {data.v5.raw.path}')
  log_info('Generating {data.v5.raw.path}')
  ...
  log_info('Current ram usage is: {mem_used()}')
  log_info('removing v4 seurat object from ram and gc()')
  rm(data.v4.raw)
  gc()
  log_info('Current ram usage is: {mem_used()}')
  
  data.v5.raw[["RNA"]] <- as(data.v5[["RNA"]], "Assay5")
  
  gc()
  
  log_info('Writing data-v5-raw to hdf5')
  time_it(
...
if (!dir.exists(hdf5.v5.raw.dir) || FORCE ) {
...
if (!file.exists(data.v5.path.normalized) || FORCE) {
  ...
  log_info("Normalized data saved\n")
} else {
  # Load the data object
  log_info("Reading normalized data\n")
...
# TODO
# analytic plots of normalization
# h5seurat save and load 
# add dimension reduction 
# add better short circuits to jump computations and read form disk instead. 
# integrate ssRNA and ATAC
